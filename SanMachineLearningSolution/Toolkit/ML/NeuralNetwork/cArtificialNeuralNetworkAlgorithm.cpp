...
ANNLAYER cArtificialNeuralNetworkAlgorithm::_GetHiddenLayer(const uint32 LayerID) const
{
	if (LayerID == 0){ return this->m_LayerArray.back(); }
	return this->m_LayerArray[LayerID];
}
bool cArtificialNeuralNetworkAlgorithm::iCreateFeatureNode(const SString &strFeatureName, const SANSTREAM &UserParam, const ANNFEATUREUPDATEFUNC FeatureUpdateFunc)
{
	...
	{
		if (this->m_FeatureArray[seek].strFeatureName == strFeatureName)
		{
			return false;
		}
	}
	ANNFEATURE Feature;
	Feature.strFeatureName = strFeatureName;
...
void cArtificialNeuralNetworkAlgorithm::iReleaseFeatureNode(const SString &strFeatureName)
{
	...
	{
		if (pFeature->strFeatureName == strFeatureName)
		{
...
void cArtificialNeuralNetworkAlgorithm::iTraining(const ANNTRAININGSET &TrainingSet, const uint32 TrainingTimes, const sfloat MinWeightChanges, const sfloat MinErrorRate, const bool bGenerateHistory)
{
	...
	for (uint32 seek = 0; seek < TrainingTimes; seek = seek + 1)
	{
		...
		{
			this->iPredict(TrainingSet[seek_set].Data);
		...
		if (CurrentErrorSum <= MinErrorRate){ break; }
	}
}
void cArtificialNeuralNetworkAlgorithm::iTrainingWithValidation(const ANNTRAININGSET &TrainingSet, const ANNTRAININGSET &ValidationSet, const uint32 TrainingTimes, const sfloat MinWeightChanges, const sfloat MinErrorRate)
{
	uint32 TrainingSetSize = TrainingSet.size();
	...
	{
		this->iPredict(ValidationSet[seek].Data);
		for (uint32 seek_node = 0; seek_node < OutputArraySize; seek_node = seek_node + 1)
		{
			Error = ValidationSet[seek].Label[seek_node] - this->m_LayerArray.back()[seek_node].SigmoidOutput;
	...
	for (uint32 seek = 0; seek < TrainingTimes; seek = seek + 1)
	{
		...
		{
			this->iPredict(ValidationSet[seek_set].Data);
			for (uint32 seek_node = 0; seek_node < OutputArraySize; seek_node = seek_node + 1)
			{
				Error = ValidationSet[seek_set].Label[seek_node] - this->m_LayerArray.back()[seek_node].SigmoidOutput;
...
vector<sfloat> cArtificialNeuralNetworkAlgorithm::iPredictC(const vector<sfloat> &InputArray, const int32 Index) const
{
	uint32 InputLayerSize = this->m_FeatureArray.size();
	if (InputArray.size() != InputLayerSize)
	{
		return vector<sfloat>();
	}
	vector<ANNLAYER> CurrentNetwork = this->m_LayerArray;
	if (Index != -1)
	{
		if (Index < this->m_HistoryList.size())
		{
			CurrentNetwork = this->m_HistoryList[Index];
		}
	}
	CurrentNetwork[0].NodeArray.resize(InputLayerSize);
	for (uint32 seek = 0; seek < InputLayerSize; seek = seek + 1)
	{
		CurrentNetwork[0][seek].SigmoidOutput = InputArray[seek];
	}
	uint32 LayerSize = CurrentNetwork.size();
	for (uint32 seek = 1; seek < LayerSize; seek = seek + 1)
	{
		uint32 NodeSize = CurrentNetwork[seek].NodeArray.size();
		//#pragma omp parallel for
		for (int32 seek_node = 0; seek_node < NodeSize; seek_node = seek_node + 1)
		{
			sfloat Net = CurrentNetwork[seek][seek_node][0].w;
			CurrentNetwork[seek][seek_node][0].x = 1.0;
			uint32 WeightVectorSize = CurrentNetwork[seek][seek_node].WeightVector.size();
			for (uint32 seek_weight = 1; seek_weight < WeightVectorSize; seek_weight = seek_weight + 1)
			{
				CurrentNetwork[seek][seek_node][seek_weight].x = CurrentNetwork[seek - 1][seek_weight - 1].SigmoidOutput;
				Net = Net + CurrentNetwork[seek][seek_node][seek_weight].w*CurrentNetwork[seek][seek_node][seek_weight].x;
			}
			CurrentNetwork[seek][seek_node].SigmoidOutput = this->_CalcSigmoidValue(Net);
		}
	}

	const uint32 OutputLayerSize = CurrentNetwork.back().NodeArray.size();

	vector<sfloat> PredictRes(OutputLayerSize);
	for (int32 seek = 0; seek < OutputLayerSize; seek = seek + 1){ PredictRes[seek] = CurrentNetwork.back()[seek].SigmoidOutput; }
...
pair<sfloat, sfloat> cArtificialNeuralNetworkAlgorithm::iFeedback(const vector<sfloat> &TrainingOutputArray)
{
	...
	for (uint32 seek = 0; seek < OutputSize; seek = seek + 1)
	{
		...
		sfloat CurrentError = TrainingOutputArray[seek] - OutputVal;
		pNode->CurrentDelta = OutputVal*(1.0 - OutputVal)*CurrentError;

		PredictError = PredictError + abs(CurrentError);
	}
	...
	for (uint32 seek = 1; seek < LayerArraySize; seek = seek + 1)
	{
		...
		for (uint32 seek_node = 0; seek_node < NodeArraySize; seek_node = seek_node + 1)
		{
			...
			for (uint32 seek_weight = 0; seek_weight < WeightVectorSize; seek_weight = seek_weight + 1)
			{
				...
				WeightChanges = WeightChanges + abs(DeltaW);
			}
		}
	}
...
void cArtificialNeuralNetworkAlgorithm::iPruning(const ANNTRAININGSET &ValidationSet)
{
	...
	for (uint32 seek_network = 0; seek_network < HistoryArraySize; seek_network = seek_network + 1)
	{
		...
		for (uint32 seek = 0; seek < ValidationSetSize; seek = seek + 1)
		{
			...
			for (uint32 seek_node = 0; seek_node < OutputArraySize; seek_node = seek_node + 1)
			{
				sfloat SubError = ValidationSet[seek].Label[seek_node] - this->m_HistoryList[seek_network].back()[seek_node].SigmoidOutput;
...
SString cArtificialNeuralNetworkAlgorithm::iPrintNeuralNetwork() const
{
	...
	/*for (uint32 seek = 0; seek < this->m_FeatureArray.size(); seek = seek + 1)
	{
		strOutput = strOutput + _SSTR("  ") + this->m_FeatureArray[seek].strFeatureName + _SSTR("\r\n");
	}
